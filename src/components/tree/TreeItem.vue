  ...
  <div>
    <v-list-tile
      ...
      <v-list-tile-content>
        ...
        <input style="background-color: #FFCCCC; border-radius: 5px; padding: 2px 8px; font-weight: bold" v-show="beginEdit" @click.stop="" :ref="model.id" v-model="model.name"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      editChild() {
        this.beginEdit = true;
        this.oldCategoryName=this.model.name;
        this.$nextTick(() => {
          this.$refs[this.model.id].focus()
        });
      },
      afterEdit() {
        if (this.beginEdit) {
          this.beginEdit = false;
          if(this.model.name!=this.oldCategoryName){
            this.handleEdit(this.model.id, this.model.name);
          }else{
            this.$message.info("无变化,无需修改");
          }
        }
      },
      handleAdd(node) {
